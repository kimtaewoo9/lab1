 ...
 *
 * 최적화 전략:
 * 1. OpenMP 병렬 처리: 'threads' 인자를 사용해 가장 바깥쪽 배치 루프를 병렬화[cite: 506, 534].
 * 2. 루프 순서 변경 (캐시 최적화): (b, i, j) 순서로 변경.
 * - 가중치 행렬 A[i*M + j]를 행(Row) 우선으로 순차 접근하여 캐시 효율 극대화.
 * 3. AVX2 SIMD 벡터화: 출력(j) 루프를 8개씩(256비트) 묶어서 처리[cite: 536].
 * 4. FMA (Fused Multiply-Add): _mm256_fmadd_ps를 사용해 곱셈과 덧셈을 단일 명령어로 처리 [cite: 331-333, 358].
 * 5. Aligned Memory 활용: 64바이트 정렬 보장을 활용해 'load_ps' (aligned load) 사용.
 * 6. Branchless ReLU: _mm256_max_ps를 사용해 'if'문 없는 ReLU 구현.
 */
#include "fc_layer.h"
...
void fc_layer(
    ...
    int N,
    int M,
    int threads      // CLI에서 전달된 스레드 수 [cite: 506]
) {
    // 1. OpenMP 스레드 수 설정 [cite: 509]
    omp_set_num_threads(threads);

    // AVX2는 256비트 = 32바이트 = float 8개를 한 번에 처리 [cite: 325]
    const int VEC_WIDTH = 8;
    
    // ReLU(max(0, x)) 연산을 위한 0으로 채워진 AVX 벡터 [cite: 339]
    const __m256 zero_vec = _mm256_setzero_ps();

    // 2. OpenMP를 사용해 가장 바깥쪽 루프 (배치 루프)를 병렬화
    // 각 입력(b)은 서로 독립적이므로 병렬화에 가장 적합
    ...
    for (int b = 0; b < num_inputs; ++b) {
        ...
        for (int j = 0; j < M; j += VEC_WIDTH) {
            // '64-byte aligned' 가 보장되므로 aligned load 사용 
